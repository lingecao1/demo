...
// Function pointer type that points to the callbackfunction which is called when a request is complete.
...
public:
	...
	void emulateLongComputationOp(unsigned int duration_sec, const std::string& raw_ip_address, unsigned short port_num, Callback callback, unsigned int request_id) {
		...
		//if the user decides to cancel the corresponding request before it completes.
		// Because active sessions list can be accessed from multiple threads, we guard it with a mutex to avoid data corruption.
...
[this, session](const boost::system::error_code& ec, std::size_t bytes_transferred) 
{
	...
	void close() {
		// Destroy work object. This allows the I/O threads to exit the event loop 
