/*
   implement async operation uinsg io_service.post() function.
*/
...
private:
    ...
    struct operation {
        ...
        operation() : service(0) {}
        io_service * service;
        op_func op;      //async operation
        completion_func completion; // the callback function
    ...
    void start() {
        ...
        // current thread just only push 'op' into vector.
        // another thread for executing aysnc op.
    ...
    void run() {
        while ( true) {
            {
                // when go out of this {}, this lock will be released.
                boost::recursive_mutex::scoped_lock lk(cs_);
                //once stop, all async operation will be given up.
                if ( !started_)
                    break; 
            }
